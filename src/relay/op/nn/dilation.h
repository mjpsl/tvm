 ...
 * \file src/relay/op/nn/dilation.h
 * \brief Properties def of convlution operator for sharing.
 */
#ifndef TVM_RELAY_OP_NN_DILATION_H_
#define TVM_RELAY_OP_NN_DILATION_H_
...
//     << " But got " << in_layout;
  ...
  if (param->kernel_size.defined() && param->channels.defined()) {
    ...
    CHECK_EQ(param->rate.size(), 2);
    Array<IndexExpr> wshape;

    // We consider kernal shape as 
    wshape = {{param->kernel_size[0], param->kernel_size[1], param->channels}};
    
    wshape = trans_kernel_layout.BackwardShape(wshape);
    channels = param->channels;
    dilated_ksize_y = 1 + (param->kernel_size[0] - 1) * param->rate[0];
    dilated_ksize_x = 1 + (param->kernel_size[1] - 1) * param->rate[1];
    ...
    if (param->kernel_size.defined()) {
      ...
      CHECK(reporter->AssertEQ(param->kernel_size[0], wshape[2]) &&
          ...
          << "Dilation2D: shape of weight is inconsistent with kernel_size, "
          << " kernel_size=" << param->kernel_size << " wshape=" << wshape;
    }
    if (param->channels.defined()) {
      CHECK(reporter->AssertEQ(param->channels, wshape[0]))
          << "Dilation2D: shape of weight is inconsistent with channels, "
          << " channels=" << param->channels << " wshape=" << wshape;
    }
	//We do not consider groups so ignore group check
    //CHECK(reporter->AssertEQ(indexdiv(dshape_nchw[1], param->groups), wshape[1]));
    channels = wshape[0];
    dilated_ksize_y = 1 + (wshape[2] - 1) * param->rate[0];
    dilated_ksize_x = 1 + (wshape[3] - 1) * param->rate[1];
  }
  // dilation
  Array<IndexExpr> oshape({dshape_nhwc[0], channels, 0, 0});

  if (!dshape_nhwc[2].as<ir::Any>()) {
    oshape.Set(2, indexdiv(dshape_nhwc[2] + param->padding[0] * 2 - dilated_ksize_y,
                           param->strides[0]) + 1);
  } else {
    oshape.Set(2, dshape_nhwc[2]);
  }

  if (!dshape_nhwc[3].as<ir::Any>()) {
    oshape.Set(3, indexdiv(dshape_nhwc[3] + param->padding[1] * 2 - dilated_ksize_x,
                           param->strides[1]) + 1);
  } else {
    oshape.Set(3, dshape_nhwc[3]);
  }
...
////////
/*
bool Dilation2DRel_conv3d(const Array<Type>& types, int num_inputs, const Attrs& attrs,
               const TypeReporter& reporter) {
  CHECK_EQ(types.size(), 3);
  const auto* data = types[0].as<TensorTypeNode>();
  const auto* weight = types[1].as<TensorTypeNode>();
  ...
  const AttrType* param = attrs.as<AttrType>();
  CHECK(param != nullptr);
  const Layout in_layout = "NHWC";
  const Layout kernel_layout= "HWC";
...
#endif  // TVM_RELAY_OP_NN_DILATION_H_
